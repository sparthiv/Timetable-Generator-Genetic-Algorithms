...
db = sql.connect(host="localhost", user="root", password=sqlpass,database="ttgen", auth_plugin='mysql_native_password')
cursor = db.cursor()
...
for i in range(classes):
    ...
    for j in range(len(teachers_for_class[i])):
        teachers_for_class[i][j] = teacher_list.index(teachers_for_class[i][j]) + 1
    ...
    for i in range(popsize):
        ...
        for j in range(classes):
            ...
            for k in range(classes):
                temp[k] = np.count_nonzero((population[i][j] - population[i][k])==0)
        ...
        if no_of_collisions != 0:
            fitness_scores[i] = (no_of_collisions * math.log(0.1, no_of_collisions))/1.5 + 200
        else:
        ...
        count_higher=0
        for i in range(popsize):
            if fitness_scores[i] < (avg_fitness + (max(fitness_scores)+min(fitness_scores))/1.5)+5:
                fitness_scores[i] = 0
            else:
        ...
        for j in range(classes):
            for k in range(days):
                for l in range(periods):
                    population[i][j][k][l] = random.choice([mate1[j][k][l], mate2[j][k][l]])
                    if random.random() < mutation:
                        population[i][j][k][l] = random.choice(teachers_for_class[j])
...
def printtofile(arr):
    ...
    for i in range (classes) :
        ws.merge_cells(start_row = ws.max_row + 2, start_column = 1, end_row =ws.max_row + 2, end_column = periods+1)
...
def initiate():
    for i in range(popsize):
        for j in range(classes):
            population[i][j] = np.random.choice(teachers_for_class[j], size = (days,periods), replace = True)
    ...
    if 200 in fitness_scores:
        for i in range(popsize):
            if fitness_scores[i] == 200:
                ...
                if timeinsec > 59:
                    print("Elapsed time:", timeinsec // 60, "mins ", timeinsec % 60, "s")
                else:
                    print(timeinsec, "s")
                os.system(filepath)
